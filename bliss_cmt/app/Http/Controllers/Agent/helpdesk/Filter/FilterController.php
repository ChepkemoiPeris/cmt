           ...
           //$table = $this->formatUserTickets($segment);
            ...
            if ($request->has('api') && $request->get('api') == '1') {
               ...
               // return $table = $this->checkRequestIsCorrect($table, $inputs);
                ...
                //$inputs = json_decode(htmlspecialchars_decode($request->get('options')),true);// return string
                // print_r($inputs);die;
                 //echo $inputs['show'][0];die;
                 //echo $inputs['assigned'][0];die;
                if(Auth::user()->role=='agent' || Auth::user()->role=='admin')
                {
                    $inputs = json_decode(htmlspecialchars_decode($request->get('options')),true);// return string
                     //$tickets_inbox=$inputs['show'][0];
                    // $tickets_unassign=$inputs['assigned'][0];
                     // print_r($inputs);die;

                    if($inputs['show'][0]=='unassign')
                     {
                       
                       $table->where('tickets.assigned_to', '=', 0)
                            ->orWhereNull('tickets.assigned_to');
                      
                        return \Ttable::genreateTableJson($table);
                     }
                     if($inputs['show'][0]=='inbox')
                     {
                       return \Ttable::genreateTableJson($table);
                     }
                     if($inputs['show'][0]=='overdue')
                     {

                       $table->whereNotNull('tickets.duedate')
       ...
       // print_r($inputs);die;
        ...
        foreach ($inputs as $key => $input) {
            if (!in_array($key, $available_options)) {
                ...
                $table = $this->filterByInputs($key, $input, $table, $mytickets);//mytickets
            }
        }
