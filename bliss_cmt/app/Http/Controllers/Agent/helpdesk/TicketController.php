...
class TicketController extends Controller
{
    ...
    public function post_newticket(CreateTicketRequest $request, CountryCode $code, $api = false)
    {
        ...
        try {
            ...
            $result = $this->create_user($email, $fullname, $subject, $body, $phone, $phonecode, $mobile_number, $helptopic, $sla, $priority, $source, $headers, $help, $assignto, $form_data, $auto_response, $status);
    ...
    public function thread($id)
    {
        ...
        if (Auth::user()->role == 'agent') {
            $dept = Department::where('id', '=', isset(Auth::user()->primary_dpt))->first();
            $tickets = Tickets::where('id', '=', $id)->first();
            if ($tickets->dept_id == isset($dept->id)) {
                $tickets = $tickets;
...
//            dd($tickets);
    ...
    public function reply(Ticket_Thread $thread, Request $request, Ticket_attachments $ta, $mail = true, $system_reply = true, $user_id = '')
    {
        ...
        try {
            ...
            $thread->poster = $fullname; 
            file_put_contents('reply.json',$fullname);
...
/*            foreach ($emails_to_be_sent as $email_data) {
    ...
    public function createTicket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status)
    {
        ...
        //date_default_timezone_set('Africa/Nairobi'); 
        $ticket_number = '';
        $location_name = $form_data['company_location'];   
        $location=DB::table('centres')->join('organization', 'organization.id', '=', 'centres.region_id')
        ->where('centres.centre','=',$location_name)->get();
       $region = '';
       $region_name='';
       foreach ($location as $val) {
        ...
        if ($max_number) {            
            ...
            $str = substr($ticket_number,0,5);// got string           
            $ticket_number = substr($ticket_number,5);//got number 
            $ticket_number = $ticket_number+1;
            $ticket_number = $str.$ticket_number; 
        } else{              
            $ticket_number = "BLISS1"; 
        } 
            //dd($ticket_number);
            
        $user_status = User::select('id','email','first_name','last_name','active')->where('id', '=', $user_id)->first();
        
        // dd($user_status->active);
        $servicedata = $form_data['service'];
        $service_issue = $form_data['service_issue']; 
        $service = Service::find($servicedata);// get service name  
        $matrixUser=DB::table('esclation_matrix')->where('companyservice','=',$service->service_name)
         ->where('company_service_issue','=',$service_issue)
         ->where('region','=',$region_name)->limit(1)->get();  
        $matuser='';
        foreach($matrixUser as $val){
            $matuser=$val->email1;
        }   
       
        $userEmail = User::where('email',$matuser)->first(); 
        $ticket = new Tickets();
        ...
        $ticket->sla = $sla;
        $ticket->assigned_to = $userEmail->id;
      ...
      //  \Event::fire('after.ticket.created', [['ticket' => $ticket, 'form_data' => $form_data]]);//i have comment this line

        // store collaborators
      //  $this->storeCollaborators($headers, $id);//comment by me
    ...
    public function close($id, Tickets $ticket)
    {
        ...
        $ticket_status->status_updated_by = Auth::user()->id;
        ...
        try {
          ...
          //  $data['name'] = $user->first_name;
           // $data['subject']= ucwords($ticket_subject)."#".$ticket_number;
           // $data['content'] ='Ticket has been closed.';
           /*Mail::send('ticket',$data, function ($m) use ($data) {
            $m->to($data['email'], $data['name'])->subject($data['subject']);
            $m->from(env('MAIL_FROM_ADDRESS'), 'Patient Relation System');
        });*/
...
//        $ticket_status = $ticket->where('id', '=', $id)->first();
    ...
    //Inprogress 
    public function inprogress($id, Tickets $ticket)
    {
        ...
        $ticket_status->status_updated_by = Auth::user()->id;
        $ticket_status->updated_at = date('Y-m-d H:i:s');
        $ticket_status->save();
        $ticket_status_message = Ticket_Status::where('id', '=', $ticket_status->status)->first();
        $thread = new Ticket_Thread();
        $thread->ticket_id = $ticket_status->id;
        ...
        return 'your ticket'.$ticket_status->ticket_number.' has been Inprogress';
    }
    // function to reassign ticket
    public function reassignForm($id , Tickets $ticket){  
        $ticket_id=$id; 
        return view('themes.default1.client.helpdesk.reassign', compact('id')); 
    }

public function reassign($id ,$ticketid, Tickets $ticket){    
    $user = DB::table('users')->select('id', 'user_name')->where('id', '=', $id)->first();
    $tkt= Tickets::where('id', '=', $ticketid)
    ->update(['assigned_to' => $id]); 
    return redirect('/ticket/inbox')->with('success','This ticket has been reassigned to '. $user->user_name); 
}
    /**
    ...
    public function InternalNote($id)
    {
        ...
        $fullname=Auth::user()->first_name." " .Auth::user()->last_name; 
        $NewThread->poster = $fullname;
        file_put_contents('internote.json',$fullname);
    ...
    public function surrender($id)
    {
        ...
        $fullname=Auth::user()->first_name." ".Auth::user()->last_name; 
        $NewThread->poster = $fullname;
        file_put_contents('surrender.json',$fullname);
        $NewThread->title = $thread->title;
        $NewThread->body = $InternalContent;
        $NewThread->save();
    ...
    public static function genreateTableJson($tickets)
    {
        return \Datatables::of($tickets)
                        ...
                        ->editColumn('title', function ($tickets) {
                         ...
                         return "<div class='tooltip1' id='tool".$tickets->id."'>
                            <a href='".route('check_ticket',[Crypt::encrypt($tickets->id)])."'>".$string."&nbsp;<span style='color:green'>".$thread_count."</span>
                            </a> <span><i style='color:green' title='".Lang::get('lang.ticket_created_source', ['source' => $source])."' class='".$css."'></i></span>".$collabString.$attachString.$due.$tooltip_script.
                                    "<span>".'</span></div>';        
                        ...
                        ->editColumn('a_uname', function ($tickets) {
                            if (is_null($tickets->assigned_to)) {
