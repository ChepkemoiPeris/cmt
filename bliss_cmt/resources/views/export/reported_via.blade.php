@php
   ...
   @endphp
    ...
    ?> 
     ...
     <table style="background-color: #18479D;color:#FFFFFF">
        ...
        <tr> <td>{{$other}}</td><td></td><td></td><td>Run Date:{{$date}}</td><td></td><td>Time:{{$time}}</td><td></td></tr>
       ...
       <tr><td>Reported Via</td><td>Status</td><td> 0-7 Days</td><td>7-15 Days</td><td>16-30 Days</td><td>>30 Days</td><td>Total Issues</td></tr>
    </table>
    
    <table border="1">
      
  
    @foreach($reportedvia as $row)
       ...
       @php
        ...
        $rowtotal = $open+$resolved+$closed;
       @endphp

    <tr>
    <td rowspan="4" valign="middle">{{$row->service_name}}</td>
   ...
   <tr>
     <td>Total</td> <td>{{$row->Open+$row->Resolved+$row->Closed}}</td>  <td>{{$row->OPen_7_15+$row->Resolved_7_15+$row->Closed_7_15}} </td> 
     <td>{{$row->OPen_16_30+$row->Resolved_16_30+$row->Closed_16_30}}</td>  <td>{{$row->OPen_30+$row->Resolved_30+$row->Closed_30}}</td>
     <td>{{$rowtotal}}</td>
   </tr>
  @endforeach
    ...
    <?php
        ...
        $totalopen=0;$totalresolved=0;$totalclosed=0;$grandtotal=0;

        $openseven_days=0;$resolvedseven_days=0;$closedseven_days=0;
        $openseven15_days=0;$resolvedseven15_days=0;$closedseven15_days=0;
        $open16_30_days=0;$resolved16_30_days=0;$closed16_30_days=0;
        $openmore_30_days=0;$resolvedmore_30_days=0;$closedmore_30_days=0;

        foreach($reportedvia as $val) {
        ...
        $count7daysTotal=$openseven_days+$resolvedseven_days+$closedseven_days;
        $count7_15daysTotal=$openseven15_days+$resolvedseven15_days+$closedseven15_days;
        $count16_30daysTotal=$open16_30_days+$resolved16_30_days+$closed16_30_days;
        $countgreater_30daysTotal=$openmore_30_days+$resolvedmore_30_days+$closedmore_30_days;
        $grandtotal = $totalopen+$totalresolved+$totalclosed;
    ...
    <tr>
    <td rowspan="3" valign="top"><strong>Total</strong></td>
