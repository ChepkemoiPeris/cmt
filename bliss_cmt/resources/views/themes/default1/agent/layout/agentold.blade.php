    ...
    <body class="skin-blue fixed">
        <div class="wrapper">
            <header class="main-header">
                <a href="http://www.faveohelpdesk.com" class="logo"><img src="{{ asset('lb-faveo/media/images/Blisslogo.jpg')}}" width="100px;"></a>
                <?php
...
//$inbox = App\Model\helpdesk\Ticket\Tickets::all();
                            ...
                            //  $unassigned = App\Model\helpdesk\Ticket\Tickets::whereNull('assigned_to')->where('status', '=', '1')->get();
                            $unassigned=DB::table('tickets')->whereNull('assigned_to')->where('status', '=', '1')->get();
...
//$inbox = App\Model\helpdesk\Ticket\Tickets::where('dept_id','',Auth::user()->primary_dpt)->get();
                            $myticket = App\Model\helpdesk\Ticket\Tickets::where('assigned_to', Auth::user()->id)->where('status', '1')->get();
                            // $unassigned = App\Model\helpdesk\Ticket\Tickets::whereNull('assigned_to')->where('status', '=', '1')->where('dept_id', '=', Auth::user()->primary_dpt)->get();
                            $unassigned=DB::table('tickets')->whereNull('assigned_to')->where('status', '=', '1')->where('dept_id', '=', Auth::user()->primary_dpt)->get();
