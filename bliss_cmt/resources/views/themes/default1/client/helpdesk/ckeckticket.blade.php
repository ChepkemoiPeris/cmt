...
<div id="content" class="site-content col-sm-12">
    ...
    <article class="hentry">
        ...
        <header class="entry-header">
            ...
            <div class="row">
                <div style="display: none;width:1118px;" id="auauth">
                     <div class="alert alert-danger alert-dismissible" >
                         <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                         <strong></strong>&nbsp;You are not Authorised.
                    </div>
                </div>

                <div style="display: none;width:1118px;" id="alert11">
                     <div class="alert alert-success alert-dismissible" >
                         <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                         <strong id="message-success1"> </strong>
                    </div>
                </div>
                ...
                <div class="col-sm-3 text-right">
                    ...
                    <div> 
                        ...
                        <div class="dropdown-menu">
                            ...
                            <a class="dropdown-item" href="#" id="inprogress">
                                <i class="fas fa-check-circle " style="color:#6495ED;"> </i> {!! Lang::get('lang.inprogress') !!}
                            </a>
                            @if(Auth()->user()->role == 'admin')
                            <a href="{{url('../ticket/reassignform/'.$tickets->id)}}" class="dropdown-item">
                            <i class="fas fa-check-circle " style="color:#F89COE;"> </i> {!! Lang::get('lang.reassign') !!}
                            </a>
                            @endif
                        </div>
                    </div>
                ...
                <div> 
                    ...
                    <div class="alert alert-secondary">
                        <div class="row">
                            ...
                            <div class="col-md-3"> 
                                <b>{!! Lang::get('lang.created_date') !!}: </b> {{date('d/m/Y h:i:s',strtotime($tickets->created_at)) }}
                            </div>
                            <div class="col-md-3"> 
                            ...
                            <div class="col-md-3">
                                <?php 
                                $response = App\Model\helpdesk\Ticket\Ticket_Thread::where('ticket_id', '=', $tickets->id)->where('is_internal', '=', 0)->get(); ?>
                                @foreach($response as $last)
                                <?php $ResponseDate = $last->created_at; ?>
                                @endforeach
                                <b>{!! Lang::get('lang.last_response') !!}: </b> {{ date('d/m/Y h:i:s',strtotime($ResponseDate))}} 
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table">
                            ...
                            <tr>
                                ...
                                @elseif($status->id == 8)
                                 ...
                                 <td title="{{$status->properties}}" style="color:blue">{{$status->name}}</td> 
                            ...
                            <tr>
                                ...
                                $ticket_form_datas = App\Model\helpdesk\Agent\ClientResult::where('ticket_id', '=', $tickets->id)->first();
                                $service = App\Model\helpdesk\Agent\Service::where('id',$ticket_form_datas['service'])->first();
                               ?>
                                <td title="{{ $service->service_name }}">{!! $service->service_name !!}</td>
                            </tr>
                        </table>
                    </div>
                    ...
                    <div class="col-md-6"> 
                        ...
                        <table class="table">
                            ...
                            <tr>
                                ...
                                <td title="{{$ticket_form_datas->service_issue}}">{{$ticket_form_datas->service_issue}}</td>
                            </tr>
                ...
                <ol class="comment-list" >
                    ...
                    <li class="comment">
                        ...
                        <article class="comment-body">
                            ...
                            <?php if (isset($role->role) && $role->role == "user") { ?> style="background-color: hsla(100, 100%, 51%, 0.15)" <?php } else { ?> style="background-color:#FFFCB3" <?php } ?>  
                            > 
                                <div class="comment-author">

                                    <img src="{{@$role->profile_pic}}"alt="" height="50" width="50" class="avatar" 
                                    <?php if (isset($role->role) && $role->role == "user") { ?>style="box-shadow: 0 1px 3px #00FF26;" <?php } else { ?> style="box-shadow: 0 1px 3px #FFEC00;" <?php } ?> 
                                    >
                                    @if(isset($role->role) && $role->role == "user")
                                ...
                                <div class="comment-metadata">
                                    ...
                                    <small class="date text-muted">
                                        ...
                                        {{ date('d/m/Y h:i:s',strtotime($conversation->created_at)) }}
                                    </small>
    ...
    <?php $ticket_form_datas = App\Model\helpdesk\Agent\ClientResult::where('ticket_id', '=', $tickets->id)->first();
                    $service = App\Model\helpdesk\Agent\Service::where('id',$ticket_form_datas['service'])->first();
        ?>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        ...
                                        <tr><td style="width: 30%">Patient Name</td><td>{{$ticket_form_datas->client_name}}</td>
                                            </tr>
                                            <tr><td style="width: 30%">Member No</td><td>{{$ticket_form_datas->member_no}}</td>
                                            </tr>
                                            <tr><td style="width: 30%">Mobile No</td><td>{{$ticket_form_datas->mobile_no}}</td>
                                            </tr>
                                            <tr><td style="width: 30%">Service</td><td>{{$service->service_name}}</td>
                                            </tr>
                                            <tr><td style="width: 30%">Service Issue</td><td>{{$ticket_form_datas->service_issue}}</td>
                                            </tr>
                                            <tr><td style="width: 30%">Location Name</td><td>{{$ticket_form_datas->company_location}}</td>
                                            </tr>
                                            <tr><td style="width: 30%">Scheme</td><td>{{$ticket_form_datas->scheme}}</td>
                                            </tr>
...
//Add text editor
    ...
    jQuery(document).ready(function () {
        ...
        $('#inprogress').on('click', function (e) {
            $.ajax({
                type: "POST",
                url: "../ticket/inprocessticket/{{$tickets->id}}",
                data:{"_token": "{{ csrf_token() }}"},
                beforeSend: function () {
                    $("#refresh").hide();
                    $("#loader").show();
                ...
                success: function (response) {
                   ...
                   // console.log(response);return false;
                    ...
                    var message = "Success! Your Ticket have been Inprogress";
                    $("#alert11").show();
                    $('#message-success1').html(message);
                    setInterval(function () {
                        $("#alert11").hide();
